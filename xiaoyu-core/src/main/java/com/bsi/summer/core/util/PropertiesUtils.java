...
public class PropertiesUtils {
	...
	public static Properties loadProperties(String... resourcesPaths) throws IOException {
	  ...
	  public static boolean save(Properties prop,String fileName){
	        boolean isOK = false;
	        try{
	            OutputStream out = new FileOutputStream(getPath()+"/"+fileName);
	            prop.store(out,null);
	            out.close();
	            isOK = true;
	        }catch(Exception e){
	            isOK = false;
	            logger.info("save properties file:"+fileName+" error!",e);
	        }
	        return isOK;
	    }
	  
	  private static String getPath() {
			return SysPath.getPath();
		}
}
